  ...
  return (
    <div className="p-5 space-y-5 text-xs">
      ...
      <div className="bg-white rounded-xl shadow p-5">
        <div className="flex justify-between items-center mb-3">
          <div>
            <h2 className="font-semibold text-gray-800 text-sm">Don gia cong ty dac biet (Thep theo KL)</h2>
            <p className="text-xs text-gray-400 mt-0.5">Ap dung cho vat lieu nhom Thep</p>
          </div>
          <div className="flex gap-2">
            {showAddCTDB && (
              ...
              }} className="text-xs bg-green-500 text-white px-3 py-1 rounded hover:bg-green-600">Luu</button>
            )}
            <button onClick={() => setShowAddCTDB(!showAddCTDB)} className="text-xs bg-blue-50 text-blue-600 border border-blue-200 px-2 py-1 rounded hover:bg-blue-100">{showAddCTDB ? 'Huy' : '+ Them'}</button>
          </div>
        </div>
        {showAddCTDB && (
          ...
          <table className="w-full text-xs">
            <thead className="sticky top-0 bg-white z-10">
              <tr className="bg-gray-50 border-b">
                ...
                <th className="px-4 py-2 text-left text-gray-500">Don gia tam don (Y/kg)</th>
                <th className="px-4 py-2 text-left text-gray-500">Don gia tam gia cong (Y/kg)</th>
                ...
                <tr key={r.id} className="border-b hover:bg-gray-50">
                  <td className="px-4 py-2">
                    {editingCTDB?.id === r.id
                      ...
                      : <span className="font-medium">{r.tenCongTy}</span>}
                  </td>
                  <td className="px-4 py-2">
                    {editingCTDB?.id === r.id
                      ...
                      : <span className="font-mono text-blue-600">{r.donGiaTamDon}</span>}
                  </td>
                  <td className="px-4 py-2">
                    {editingCTDB?.id === r.id
                      ...
                      : <span className="font-mono text-orange-600">{r.donGiaTamGiaCong}</span>}
                  </td>
                  <td className="px-4 py-2">
                    {editingCTDB?.id === r.id
                      ? <div className="flex gap-1">
                          <button onClick={async () => {
                            ...
                            setCongTyDacBiet((p:any) => p.map((x:any) => x.id === r.id ? editingCTDB : x))
                            setEditingCTDB(null)
                          }} className="text-green-600 text-xs">OK</button>
                          <button onClick={() => setEditingCTDB(null)} className="text-gray-400 text-xs">X</button>
                        </div>
                      : <div className="flex gap-1">
                          <button onClick={() => setEditingCTDB({...r})} className="text-orange-400 hover:text-orange-600 text-xs">sua</button>
                          <button onClick={async () => {
                            await fetch('/api/cong-ty-dac-biet/' + r.id, { method: 'DELETE' })
                            setCongTyDacBiet((p:any) => p.filter((x:any) => x.id !== r.id))
                          }} className="text-red-400 hover:text-red-600 text-xs">xoa</button>
                        </div>}
                  </td>
                </tr>
              ))}
              {congTyDacBiet.length === 0 && (
                <tr><td colSpan={4} className="px-4 py-6 text-center text-gray-300">Chua co cong ty nao</td></tr>
              )}
            </tbody>
          </table>
        </div>
